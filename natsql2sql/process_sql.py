...
def get_schema(db,table_json=None):
    ...
    try:
        ...
        if not table_json:
            raise "database unfine exception!"
    if not schema and table_json:
        for i, table in enumerate(table_json["table_names_original"]):
            table_name = table.lower()
            schema[table_name]=[]
            for col in table_json["column_names_original"]:
                if col[0] == i:
                    schema[table_name].append(col[1].lower())
        ...
        if key not in schema.idMap and toks[start_idx+1] == "(":
                ...
                for i in range(start_idx+1,len(toks)):
                    ...
                    if new_key in schema.idMap:
                        mapped_entities.append((start_idx, new_key.replace(".","@")))
