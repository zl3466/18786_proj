...
def create_training_arguments(config: QwenLoraConfig, eval_dataset: Optional[Dataset] = None):
    """Create SFT training config (trl SFTConfig extends TrainingArguments)."""
    _cfg_params = inspect.signature(SFTConfig.__init__).parameters
    _length_kw = (
        {"max_length": config.MAX_SEQ_LENGTH}
        if "max_length" in _cfg_params
        else {"max_seq_length": config.MAX_SEQ_LENGTH}
    )
    if config.USE_WANDB:
    ...
    if config.WANDB_RUN_NAME:
        _run_kw["run_name"] = config.WANDB_RUN_NAME
    return SFTConfig(
